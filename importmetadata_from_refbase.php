...
/* 
   26.01.2018, ss029:
   * This file has been modified to allow for the import of some additional fields from refbase
   * MODS XML was exported from refbase and converted to RIS using an up-to-date version of bibutils
   * Following additional fields were added to (or overwritten in) the RIS file directly from the refbase database using a PHP script
     - AD: address
     - N1: notes, extended by an import remark with infos, who originally added the ref and who updated it last
     - U1: contribution_id (IBT or empty, stating whether a ref is an IBT publication or not)
     - U2: thesis (one of Bachelor's thesis, Master's thesis, Diploma thesis, Ph.D. thesis, Doctoral thesis or Habilitation thesis)
     - U3: created_date (date, when the ref was added to refbase - this is used as addition date in I-Librarian)
   * The T2 field is used as journal instead of secondary_title
   * The original cite_key is imported as bibtex field
*/
...
if (isset($_SESSION['auth']) && ($_SESSION['permissions'] == 'A' || $_SESSION['permissions'] == 'U')) {
    ...
    if (isset($_POST['form_sent']) && isset($_POST['file_type']) && (isset($_FILES['form_import_file']) && is_uploaded_file($_FILES['form_import_file']['tmp_name']) || !empty($_POST['form_import_textarea']))) {
        ...
        $query = "INSERT INTO library (file, authors, affiliation, title, journal, year, addition_date, abstract, rating, uid, volume, issue, pages, secondary_title, tertiary_title, editor,
                                        url, reference_type, publisher, place_published, keywords, doi, authors_ascii, title_ascii, abstract_ascii, added_by, custom1, custom2, bibtex, bibtex_type)
            ...
            if (!empty($file_contents)) {
                ...
                foreach ($file_records as $record) {
                    ...
                    foreach ($record_array as $line) {
                        ...
                        if (strpos($line, "T1") === 0 || strpos($line, "TI") === 0) {
                        ...
                        } elseif (strpos($line, "JA") === 0 || strpos($line, "J2") === 0 || strpos($line, "T2") === 0) {
                            $journal_match[0] = trim(substr($line, 6));
                        } elseif (strpos($line, "JF") === 0 || strpos($line, "JO") === 0 || strpos($line, "BT") === 0) {
                            ...
                            $bibtex_match[0] = trim(substr($line, 6));
                        } elseif (strpos($line, "U1") === 0) {
                            $custom1_match[0] = trim(substr($line, 6));
                        } elseif (strpos($line, "U2") === 0) {
                            $custom2_match[0] = trim(substr($line, 6));
                        } elseif (strpos($line, "U3") === 0) {
                            $addition_date_match[0] = trim(substr($line, 6));
                    ...
                    if (!empty($type_match[0])){
	                    ...
	                    $bibtex_type = convert_type(trim($type_match[0]), 'ris', 'bibtex');
                    }
                    ...
                    if (!empty($custom1_match[0])) {
                        $custom1 = trim($custom1_match[0]);
                    }
                    
                    $custom2 = '';

                    if (!empty($custom2_match[0])) {
                        $custom2 = trim($custom2_match[0]);
                    ...
                    if (!empty($addition_date_match[0])) {
                        $addition_date = trim($addition_date_match[0]);
                    }
        ...
        for ($i = 1; $i <= $item_count; $i = $i + 1000) {
            ...
            while ($tmprow = $tmpresult->fetch(PDO::FETCH_ASSOC)) {
                ...
                $custom1 = $tmprow['custom1'];
                $custom2 = $tmprow['custom2'];
